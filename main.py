...
def best_recurse_cr(s: float, x: float, y: float, num_turns: int, opt: Callable[[float], float]) -> Tuple[List[Tuple[float, float]], float, float]:
    ...
    for m in np.linspace(s, 1, N_STEPS):
        new_x = x + ((m-x)/num_turns)
        d2new_x = np.sqrt((x-new_x)**2 + (y-new_y)**2)

        # if robot fails after helper reaches new_x, recurse
        new_s = min(1, s + d2new_x) # new starting point for robot (it has traveled while helper moved to new_x)
        _points, _new_x_worst_t, _cr = best_recurse_cr(new_s, new_x, new_y, num_turns-1, opt=opt)
        points = [(new_x, new_y)] + _points
        new_x_worst_t = d2new_x + _new_x_worst_t

        # get cr if robot fails before helper reaches new_x
        for t in np.linspace(0, d2new_x, N_STEPS): # robot fails before helper reaches new_x
